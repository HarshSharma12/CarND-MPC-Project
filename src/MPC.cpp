...
class FG_eval {
    ...
    void operator()(ADvector & fg,
        ...
        for (unsigned int t = 0; t < N - 1; t++) {
            ...
            fg[1 + psi_start + t + 1] = psi1 - (psi0 + v0 * delta0 * dt / Lf);
            fg[1 + v_start + t + 1] = v1 - (v0 + a0 * dt);
            fg[1 + cte_start + t + 1] = cte1 - ((polyeval(x0) - y0) + (v0 * CppAD::sin(epsi0) * dt));
            fg[1 + epsi_start + t + 1] = epsi1 - ((psi0 - CppAD::atan(d_polyeval(x0))) + v0 * delta0 * dt / Lf);
